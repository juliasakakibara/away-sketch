...
void setup() {
  ...
  if (!HTS.begin()) {
    ...
    Serial.println("Please, check if your board is the Nano 33 BLE Sense.");
    htsSensorFail = true;
  }
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (currentMillis - startMillis >= 1000) {
        if(!htsSensorFail) {
          // if hts sensor is present, read temperature value every 1 second
          temperature = (int) HTS.readTemperature();
        } else { 
          // if hts sensor is not present or not working, generate random values
          temperature = (int) random(15, 40);
        }
